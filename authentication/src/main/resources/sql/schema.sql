...
CREATE TABLE `tb_authentication_info` (
  ...
  `sync_status` tinyint DEFAULT '0' COMMENT '同步 es 状态：0.处理中，1.成功，99.失败',
...
CREATE TABLE `tb_console_user_resource` (
  `user_id` int NOT NULL COMMENT 'tb_console_user 外键',
  `resource_id` int NOT NULL COMMENT 'tb_resource 外键',
...
CREATE TABLE `tb_group_console_user` (
  `group_id` int NOT NULL COMMENT 'tb_group 表外键',
  `user_id` int NOT NULL COMMENT 'tb_console_user 表外键',
...
CREATE TABLE `tb_group_resource` (
  `group_id` int NOT NULL COMMENT 'tb_group 外键',
  `resource_id` int NOT NULL COMMENT 'tb_resource_外键',
...
CREATE TABLE `tb_member_user_initialization` (
  ...
  `update_version` int NOT NULL DEFAULT '1' COMMENT '更新版本号',
  `user_id` int NOT NULL COMMENT '用户 id',
  `modify_password` tinyint NOT NULL COMMENT '是否可更新密码：1.是、0.否',
  `modify_username` tinyint NOT NULL COMMENT '是否客更新登录账户：1.是、0.否',
  PRIMARY KEY (`id`) USING BTREE,
...
INSERT INTO `authentication`.`tb_group` (`id`, `creation_time`, `name`, `authority`, `source`, `parent_id`, `removable`, `modifiable`, `status`, `remark`) VALUES (1, '2021-09-13 13:24:01.080', '超级管理员', 'ADMIN', 'Console', NULL, 0, 1, 1, NULL);

INSERT INTO `authentication`.`tb_console_user` (`id`, `creation_time`, `email`, `password`, `status`, `username`, `real_name`, `remark`) VALUES (1, '2021-08-18 09:40:46.953', 'admin@domian.com', '$2a$10$U2787VFuFP9NMyxwdsP1bOmtvofTgwU5nLcdV7Gj3ZyhdiZO.T8mG', 1, 'admin', '超级管理员', NULL);

INSERT INTO `authentication`.`tb_group_console_user` (`group_id`, `user_id`) VALUES (1, 1);
