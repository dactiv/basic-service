...
) ENGINE = InnoDB COMMENT = '认证信息表' ROW_FORMAT = Dynamic;
...
CREATE TABLE `tb_console_user`  (
  ...
  UNIQUE INDEX `ux_email`(`email`) USING BTREE
) ENGINE = InnoDB COMMENT = '后台用户表' ROW_FORMAT = Dynamic;
...
) ENGINE = InnoDB COMMENT = '后台用户与资源关联表' ROW_FORMAT = Dynamic;
...
CREATE TABLE `tb_group`  (
  ...
  `can_be_delete` tinyint(4) NOT NULL COMMENT '是否可删除:0.否、1.是',
  `can_be_modify` tinyint(4) NOT NULL COMMENT '是否可修改:0.否、1.是',
  `status` tinyint(4) NOT NULL DEFAULT 1 COMMENT '状态:0.禁用、1.启用',
  `creation_time` datetime(0) NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `remark` varchar(128) DEFAULT NULL COMMENT '备注',
  PRIMARY KEY (`id`) USING BTREE,
  UNIQUE INDEX `ux_name`(`name`) USING BTREE
) ENGINE = InnoDB COMMENT = '用户组表' ROW_FORMAT = Dynamic;
...
) ENGINE = InnoDB COMMENT = '后台用户与用户组关联表' ROW_FORMAT = Dynamic;
...
CREATE TABLE `tb_group_member_user`  (
  ...
  `user_id` int(11) NOT NULL COMMENT 'tb_console_user 表外键',
  PRIMARY KEY (`group_id`, `user_id`) USING BTREE
) ENGINE = InnoDB COMMENT = '会员用户与组关联表' ROW_FORMAT = Dynamic;
...
) ENGINE = InnoDB COMMENT = '用户组与资源关联表' ROW_FORMAT = Dynamic;
...
CREATE TABLE `tb_member_user`  (
  ...
  UNIQUE INDEX `ux_username`(`username`) USING BTREE,
  UNIQUE INDEX `ux_email`(`email`) USING BTREE,
  UNIQUE INDEX `ux_phone`(`phone`) USING BTREE
) ENGINE = InnoDB COMMENT = '会员用户表' ROW_FORMAT = Dynamic;
...
CREATE TABLE `tb_member_user_initialization`  (
  ...
  PRIMARY KEY (`id`) USING BTREE,
  UNIQUE INDEX `ux_user_id`(`user_id`) USING BTREE
) ENGINE = InnoDB COMMENT = '会员用户初始化表' ROW_FORMAT = Dynamic;
...
CREATE TABLE `tb_resource`  (
  ...
  `remark` varchar(128) DEFAULT NULL COMMENT '备注',
  PRIMARY KEY (`id`) USING BTREE,
  UNIQUE INDEX `ux_[code]_[application_name]_[type]_[source]_[parent_id]`(`code`, `application_name`, `source`, `parent_id`, `type`) USING BTREE
) ENGINE = InnoDB COMMENT = '资源表' ROW_FORMAT = Dynamic;

INSERT INTO `tb_console_user`(`id`, `email`, `password`, `status`, `username`, `real_name`, `creation_time`, `remark`) VALUES (1, 'admin@domian.com', '$2a$10$U2787VFuFP9NMyxwdsP1bOmtvofTgwU5nLcdV7Gj3ZyhdiZO.T8mG', 1, 'admin', '超级管理员', '2020-03-01 21:57:46', NULL);

INSERT INTO `tb_member_user`(`creation_time`, `username`, `password`, `email`, `phone`, `status`) VALUES ('2020-04-16 17:21:56', 'maurice', '$2a$10$U2787VFuFP9NMyxwdsP1bOmtvofTgwU5nLcdV7Gj3ZyhdiZO.T8mG', '', '18776974353', 1);

INSERT INTO `tb_member_user_initialization`(`user_id`, `modify_password`, `modify_username`, `build_email`) VALUES (1, 0, 0, 0);

INSERT INTO `tb_group`(`id`, `name`, `authority`, source, `parent_id`, `status`, `can_be_delete`, `can_be_modify`, `creation_time`, `remark`) VALUES (1, '超级管理员', 'ADMIN', 'Console', NULL, 0, 0, 1, '2020-03-01 21:56:25', NULL);
INSERT INTO `tb_group`(`id`, `name`, `authority`, source, `parent_id`, `status`, `can_be_delete`, `can_be_modify`, `creation_time`, `remark`) VALUES (2, '普通用户', 'ORDINARY', 'UserCenter', NULL,  0, 0, 1, '2020-03-01 21:56:25', NULL);

INSERT INTO `tb_group_console_user`(`group_id`, `user_id`) VALUES (1, 1);

INSERT INTO `tb_group_member_user`(`group_id`, `user_id`) VALUES (2, 1);
